document.addEventListener('DOMContentLoaded', () => {
    const controls = document.querySelectorAll('.view-btn, .toggle-btn, .btn.toggle-btn');
    ...
    function hidePanel(panel) {
        ...
        if (id) {
            const btns = document.querySelectorAll(
                `[data-target="#${id}"], [data-target="${id}"], [data-panel="${id}"], a[href="#${id}"], a[href="${id}"]`
            );
    ...
    controls.forEach(ctrl => {
        ctrl.addEventListener('click', (e) => {
            ...
            let target = ctrl.dataset.target || ctrl.getAttribute('data-panel') || ctrl.getAttribute('href') || '';
            if (!target) return;
